...
class Block(nn.Module):
    ...
    def forward(self, x, cond=None, scale_shift=None):
        x = self.proj(x)
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, label_channel=10, time_emb_dim=None, groups=8):
        super().__init__()
        ...
        self.block1 = Block(dim, dim_out, groups=groups, label_channel=label_channel)
        self.block2 = Block(dim_out, dim_out, groups=groups, label_channel=label_channel)
    ...
    def forward(self, x, cond=None, time_emb=None, ):
        ...
        h = self.block1(x, cond, scale_shift=scale_shift)
...
class Unet3DSPADE(nn.Module):
    def __init__(
        ...
        block_klass_cond = partial(block_klass, time_emb_dim=cond_dim, label_channel=num_semantic_map_channels)
    ...
    def forward(
        ...
        if self.has_cond:
            ...
            if self.use_bert_text_cond:
                torch.where(rearrange(mask, 'b -> b 1'), self.null_cond_emb, cond)
            else:
                # use semantic conditioning, first by passing through a semantic embedding layer
        ...
        for block1, block2, spatial_attn, temporal_attn, downsample in self.downs:
            ...
            h.append(x)
            x = downsample(x)
        x = self.mid_block1(x, time_emb=t, cond=cond)
        x = self.mid_spatial_attn(x)
        x = self.mid_temporal_attn(x, pos_bias=time_rel_pos_bias, focus_present_mask=focus_present_mask)
        x = self.mid_block2(x, time_emb=t, cond=cond)

        for block1, block2, spatial_attn, temporal_attn, upsample in self.ups:
            x = torch.cat((x, h.pop()), dim=1)
            x = block1(x, time_emb=t, cond=cond)
            x = block2(x, time_emb=t, cond=cond)
            x = spatial_attn(x)
            # save x after spatial attention at size torch.Size([8, 32, 16, 128, 128]) channel 1 for visualization
            x = temporal_attn(x, pos_bias=time_rel_pos_bias, focus_present_mask=focus_present_mask)
            x = upsample(x)
