...
class DummyDataset(Dataset):
    def __init__(self, image_size, channels=3, num_frames=16):
        super().__init__()
    ...
    def __getitem__(self, index):
        return {
            "image": torch.randn(self.channels, self.num_frames, self.image_size, self.image_size),
            "cond": torch.randint(0, self.num_classes, (self.num_frames, self.image_size, self.image_size))
        }
...
class EchoVideoDataset(Dataset):
    def __init__(
        ...
        self.seg_maps = self._get_video_data(self.folder / 'seg_maps_cone')
