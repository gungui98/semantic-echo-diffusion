...
class Trainer(object):
    def __init__(
        ...
        if data_mode == "echo":
            self.train_ds = EchoVideoDataset(folder, image_size, channels=channels,
                                             num_frames=num_frames, split="train")
            self.test_ds = EchoVideoDataset(folder, image_size, channels=channels,
    ...
    def train(
        ...
        while self.step < self.train_num_steps:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                all_videos_list = [x[0] for x in preds], [x[1] for x in preds]
                all_videos_list = torch.cat(all_videos_list, dim=0)

                video_path = str(self.results_folder / str(f'{milestone}.gif'))
                plot_seq(all_videos_list, cond, video_path)
